...
def main():
    ...
    if args.arch == 'resnet18_base':
        model = nn.DataParallel( resnet.resnet18(pretrained=not args.no_pretrain if not args.resume else False, num_classes=6, use_att=args.use_att, att_mode=args.att_mode).to(device) )
